...
static void _protocol_RequestParsing(void){
	...
	//--------------------------
	//Проверка CRC.
	uint8_t crcCalc = CRC8_FastCalculate((uint8_t*)request, request->Count+1);
	uint8_t crcReq  = request->Payload[request->Count-1];
	if(crcCalc != crcReq) return;//если CRC не совпадает то выходим.

	//LED_ACT_Toggel();//Отладка. Индикация приема пакета.
...
//		break;
		...
		case(cmdGetEncoderPosition):
			...
			*(uint32_t*)&response->Payload[2]  = ENCODER_GetCode();	     //u32 data
...
void PROTOCOL_I2C_Init(void){
	...
	I2cWire.i2cRxCallback = _protocol_RequestParsing;
	I2cWire.i2cTxCallback = _protocol_TxParsing;
	I2C_IT_Init(&I2cWire);
}
//**********************************************************
