...
void GPIO_MCU_Init(void){
	...
	//MCU_EN - PB2 - выход. Цепь включения питания платы MCUv7.
	GPIO_InitForOutputPushPull(MCU_EN_GPIO_PORT, MCU_EN_GPIO_PIN);

	//PWR_BTN_LED - PB9 - выход. Светодиод кнопки включения питания.
	GPIO_InitForOutputPushPull(PWR_BTN_LED_GPIO_PORT, PWR_BTN_LED_GPIO_PIN);

	//MCU_PWR_PBN - PB7 - вход. Кнопка включения питания.
	GPIO_InitForInputPullUp(MCU_PWR_BTN_GPIO_PORT, MCU_PWR_BTN_GPIO_PIN);
...
uint32_t GPIO_MCU_GetPwrButton(void){
	...
	return (MCU_PWR_BTN_GPIO_PORT->IDR & (1<<MCU_PWR_BTN_GPIO_PIN));
}
//**********************************************************
