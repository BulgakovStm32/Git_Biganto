...
void speed_cntr_Move(signed int step, unsigned int accel, unsigned int decel, unsigned int speed){
	...
	unsigned int max_s_lim;//! Number of steps before we hit max speed.
	...
	else if(step != 0)
	{
		...
		if(accel_lim <= max_s_lim) srd.decel_val = accel_lim - step;
...
void speed_cntr_Init_Timer1(void){
	...
	TCCR1B = (1<<WGM12); // Timer/Counter 1 in mode 4 CTC (Not running).
...
__interrupt void speed_cntr_TIMER1_COMPA_interrupt(void){
	...
	static int last_accel_delay;	   // Remember the last step delay used when accelrating.
	...
	switch(srd.run_state){
		...
		case DECEL:
			...
			if(srd.accel_count >= 0) srd.run_state = STOP;
		break;
		//-------------------
	}
