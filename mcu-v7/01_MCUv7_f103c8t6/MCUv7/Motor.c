...
//*******************************************************************************************
//*******************************************************************************************
static void _motor_TimerInit(uint32_t freg){

	TIM1_Init(freg);
}
//**********************************************************
static void _motor_TimerEnable(void){

	TIMx_Enable(MOTOR_TIMER); //Counter enable
}
//**********************************************************
static void _motor_TimerDisable(void){

	TIMx_Disable(MOTOR_TIMER); //Counter disable
}
//**********************************************************
static void _motor_SetTimerCount(uint32_t val){

	TIMx_SetARR(MOTOR_TIMER, val-1);
}
//**********************************************************
//Направление вращения.
static void _motor_Direction(uint32_t dir){
 ...
 */
static void _motor_SpeedCntrMove(int32_t step, uint32_t accel, uint32_t decel, uint32_t speed){
...
//	else _motor_Direction(MOTOR_CCW); //против часовой стрелки
	...
	if(step == 1)
	{
		...
		speedRamp.state   	  = DECEL; // ...in DECEL state.
...
void MOTOR_Init(void){
	...
	//Частота работы базового таймера для генерации шагов.
	...
	MOTOR_SetMicrostepMode(MOTOR_DEFAULT_MICROSTEP);   		//Кол-во микрошагов.
	MOTOR_SetReducerRate(MOTOR_DEFAULT_REDUCER_RATE);		//передаточное число редуктора.
	MOTOR_SetAccelerationTime(MOTOR_DEFAULT_ACCEL_TIME_mS);	//время ускорения в милисекундах.
	MOTOR_SetVelocity(MOTOR_DEFAULT_RPM);		   			//cкорость, оборотов в минуту (RPM).
	speedRamp.state = STOP;						   			//состяние рампы "STOP".
...
void MOTOR_SetMicrostepMode(MotorStepMode_t uSteps){
	...
	if(speedRamp.state != STOP) return;//при вращении ничего не делаем и выходим
	uStepsMode = uSteps;
	switch(uSteps){
		//-------------------
		case(STEP_1):
...
void MOTOR_CalcAndStartRotation(void){
	...
	_motor_SpeedCntrMove(steps, 	//param step  - Number of steps to move
						...
						(accel*2),  //accel, 	//param decel - Decelration to use, in 0.01*rad/sec^2.
...
//**********************************************************
void MOTOR_StopRotation(void){

	speedRamp.state = STOP;
}
//*******************************************************************************************
//*******************************************************************************************
//*******************************************************************************************
...
void MOTOR_TimerITHandler(void){
	...
	switch(speedRamp.state){
		...
		case DECEL:
			...
			if(speedRamp.decel_val <= 0) speedRamp.state = STOP;
		break;
		//-------------------
	}
