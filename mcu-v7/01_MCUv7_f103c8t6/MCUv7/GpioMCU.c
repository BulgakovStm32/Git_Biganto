...
void GPIO_MCU_Init(void){
	...
	// MCU_EN - PB2
	MCU_POWER_ON();	//это нужно для перехода из загрузчика без отключения питания MCU.
	...
	// PWR_BTN_LED - PB9
	PWR_BTN_LED_High();	//это нужно для перехода из загрузчика без мерцания светодиода кнопки питания.
	...
	// BB_PWR_EN - PA11 - выход push/pull.
	BB_PWR_EN_High();	//это нужно что бы питание БигБорд не отключалось после перехода из загрузчика.
	STM32_GPIO_InitForOutputPushPull(BB_PWR_EN_GPIO_PORT, BB_PWR_EN_GPIO_PIN);
}
//**********************************************************
