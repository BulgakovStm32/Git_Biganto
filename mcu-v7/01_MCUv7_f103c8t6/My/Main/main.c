...
void Task_Motor(void){
	...
	uint32_t angle		    = 360*2; 	//Угол на который нужно переместить вал камеры в градусах.
...
//	LAMP_PWM_High();
	...
	//***********************************************
	//Иницияализация ШИМ для управления LAMP
	//Используется вывод PB1(TIM3_CH4).
	TIM3_InitForPWM();
	...
	//Ини-я диспетчера.
	RTOS_Init();
	RTOS_SetTask(Task_TEMPERATURE_Read, 1000, 500);//запуск задачи через 1000мс и с периодом повторения 500мс
//	RTOS_SetTask(Task_POWER_Check,      0, 100);   //запуск задачи через 0мс и с периодом повторения 100мс
	RTOS_SetTask(Task_GPS,         		1000, 500);
