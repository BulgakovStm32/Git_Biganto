...
static uint32_t _i2c_LongWait(I2C_TypeDef *i2c, uint32_t flag){
	...
	uint32_t count = DELAY_microSecCount();
	//---------------------
	while(!(i2c->SR1 & flag))//Ждем отпускания флага.
	{
		if((DELAY_microSecCount() - count) >= I2C_WAIT_TIMEOUT_uS) return 1;
	}
	return 0;
}
//**********************************************************
...
//**********************************************************
static void _i2c_ClearSRxFlags(I2C_TypeDef *i2c){
...
void I2C_IT_Init(I2C_IT_t *i2cIt){
	...
	if(i2cIt->i2c == I2C1)
	{
		NVIC_SetPriority(I2C1_EV_IRQn, 13);//Приоритет прерывания.
		NVIC_SetPriority(I2C1_ER_IRQn, 14);//Приоритет прерывания.
	...
	{
		NVIC_SetPriority(I2C2_EV_IRQn, 13);//Приоритет прерывания.
		NVIC_SetPriority(I2C2_ER_IRQn, 14);//Приоритет прерывания.
...
void I2C_IT_Master(I2C_IT_t *i2cIt){

}
//*******************************************************************************************
//*******************************************************************************************
void I2C_IT_Slave(I2C_IT_t *i2cIt){

	I2C_TypeDef *i2c = i2cIt->i2c;
	static volatile uint32_t i2c_mode = 0;
	//------------------------------
	/* ADDR set --------------------------------------------------------------*/
	if((i2c->SR1 & I2C_SR1_ADDR) && (i2c->CR2 & I2C_CR2_ITEVTEN))
	{
		_i2c_ClearSRxFlags(i2c);	//сбрасываем I2C_IT_ADDR чтением SR1 и SR2
		i2c->CR2 |= I2C_CR2_ITBUFEN;//включаем прерывание I2C_IT_BUF - вначале было отключено
		i2cIt->ITState = I2C_IT_STATE_ADDR_MATCH;
		//Смотрим что хочет Мастер.
		if(i2c->SR2 & I2C_SR2_TRA) i2c_mode = I2C_MODE_READ; //От Мастера пришла команда на чтение - SLA+Rd
		else					   i2c_mode = I2C_MODE_WRITE;//От Мастера пришла команда на запись - SLA+Wr
		//Действие при совпадении адреса.
		if(i2cIt->i2cSlaveAddrMatchCallback != 0) i2cIt->i2cSlaveAddrMatchCallback();
	}
	/* STOPF set --------------------------------------------------------------*/
	else if((i2c->SR1 & I2C_SR1_STOPF) && (i2c->CR2 & I2C_CR2_ITEVTEN))
	{
		_i2c_ClearSRxFlags(i2c);     //сбрасываем I2C_IT_STOPF чтением SR1
	...
	else if(i2c_mode == I2C_MODE_READ) //От Мастера пришла команда на чтение - SLA+Rd
	{
		   ...
		   !(i2c->SR1 & I2C_SR1_BTF) )
		{
			...
			{
				i2cIt->i2cSlaveTxCpltCallback();//Обработки события передачи буфера
				i2cIt->txBufSize  = 0;
				i2cIt->txBufIndex = 0;
			}
		}
	...
	else //От Мастера пришла команда на запись - SLA+Wr
	{
		...
		else if((i2c->SR1 & I2C_SR1_BTF) && (i2c->CR2 & I2C_CR2_ITEVTEN))
		{
			...
			*(i2cIt->pRxBuf + i2cIt->rxBufIndex) = (uint8_t)i2c->DR;
			 i2cIt->rxBufIndex++;
			 //смотрим сколько байт нужено принять.
			 //второй байт пакета - это размер принимаемого пакета. подробности в описании протокола.
			 if(i2cIt->rxBufIndex == 2) i2cIt->rxBufSize = i2cIt->pRxBuf[1] + 2;
		}
		else
...
//	I2C_TypeDef *i2c = i2cIt->i2c;
//	//------------------------------
//	//Start bit (Master mode) - Сформирована START последовательность.
