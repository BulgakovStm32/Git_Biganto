...
//**********************************************************
// Function    : _oneWire_ResetAndPresencePulse
// Description : Последовательность инициализации: импульсы сброса и присутствия.
// Parameters  : port - порт микроконтроллера, используемый для шины 1-Wire
//				 pin  - номер порта (0 - 15)
// RetVal      : 0 - нет устройста на шине; 1 - получем импульс присутствия от устр-ва.
//*****************************************
static uint8_t _oneWire_ResetAndPresencePulse(GPIO_TypeDef *const port, uint32_t pin){

	uint8_t presence = 1;
	pin = (1 << pin);
	//---------------------
	if(!port) return 0;	//Проверка. Не опрделен порт - значит нет датчика.
	...
	if(port->IDR & pin) presence = 0;

	_oneWire_usDelay(500);		 //подождём, так как могут быть неточности в задержке.
	return presence;
}
//**********************************************************
// Function    : _oneWire_SendBit
// Description : Запись бита на шину.
// Parameters  : port - порт микроконтроллера, используемый для шины 1-Wire
//				 pin  - номер порта
//				 bit  - значение записываемого бита (0 или 1)
// RetVal      :
//*****************************************
static void _oneWire_SendBit(GPIO_TypeDef *const port, uint32_t pin, uint8_t bit){
...
//**********************************************************
// Function    : _oneWire_SendByte
// Description : Запись байта на шину
// Parameters  : port - порт микроконтроллера, используемый для шины 1-Wire
//				 pin  - номер порта
//				 byte - байт, передаваемый на шину
// RetVal      :
//*****************************************
static void _oneWire_SendByte(GPIO_TypeDef *const port, uint32_t pin, uint8_t byte){

	for(uint32_t i = 0; i < 8; i++)
	{
		_oneWire_SendBit(port, pin, ((byte >> i) & 0x01));
	}
}
//**********************************************************
// Function    : _oneWire_ReadBit
// Description : Чтение бита.
// Parameters  : port - порт микроконтроллера, используемый для шины 1-Wire
//				 pin  - номер порта
// RetVal      : значение прочитанного бита (0 или 1)
//*****************************************
...
//*****************************************
static uint8_t _oneWire_ReadByte(GPIO_TypeDef *const port, uint32_t pin){
...
//**********************************************************
// Function    : _oneWire_ReadData
// Description : Чтение заданного количества байтов в буфер
// Parameters  : port - порт микроконтроллера, используемый для шины 1-Wire
//				 pin  - номер порта
//				 buf  - указатель на буфер, куда читает
//				 size - кол-во считываемых байтов (от 1 до 256)
// RetVal      :
//*****************************************
static void _oneWire_ReadData(GPIO_TypeDef *const port, uint32_t pin, uint8_t *buf, uint8_t size){

	for(uint8_t i = 0; i < size; i++)
	{
		buf[i] = _oneWire_ReadByte(port, pin);
	}
}
//*******************************************************************************************
//*******************************************************************************************
...
void ONE_WIRE_ReadDeviceData(ds18b20_t *device){
	...
	//---------------------
	//Чтение блокнота (9 байт) из DS18B20
	_oneWire_ReadData(ONE_WIRE_PORT, ONE_WIRE_PIN, device->scratchPad.Blk, sizeof(device->scratchPad.Blk));
...
void TEMPERATURE_SENSE_Init(void){
	...
	ONE_WIRE_SearchDevices(devices, 2, &numberDevices);
	...
	while(cnt > 0)
	{
		...
		TEMPERATURE_SENSE_SetResolution(cnt, DS18B20_REOLUTION_12_BIT);

		//Запуск преобразования температуры
		ONE_WIRE_SendCmd(devices[cnt-1].id, CONVERT_T);
